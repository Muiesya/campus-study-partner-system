...
public class GraphSyncService {
    ...
    private final Logger log = LoggerFactory.getLogger(GraphSyncService.class);
    ...
    public void syncStudent(Student student) {
        runSafe(() -> neo4jClient.query("MERGE (s:Student {id:$id}) SET s.name=$name, s.major=$major")
                .bindAll(Map.of(
                        "id", student.getId(),
    ...
    public void syncCourse(Course course) {
        runSafe(() -> neo4jClient.query("MERGE (c:Course {id:$id}) SET c.name=$name, c.description=$description")
                .bindAll(Map.of(
                        "id", course.getId(),
                        "name", course.getName(),
    ...
    public void syncTag(InterestTag tag) {
        runSafe(() -> neo4jClient.query("MERGE (t:Tag {id:$id}) SET t.name=$name")
                .bindAll(Map.of(
                        "id", tag.getId(),
                        "name", tag.getName()))
                .run());
    ...
    public void linkStudentCourse(Student student, Course course) {
        syncStudent(student);
        syncCourse(course);
        runSafe(() -> neo4jClient.query("MATCH (s:Student {id:$sid}), (c:Course {id:$cid}) MERGE (s)-[:ENROLLED_IN]->(c)")
    ...
    public void linkStudentTag(Student student, InterestTag tag) {
        syncStudent(student);
        syncTag(tag);
        runSafe(() -> neo4jClient.query("MATCH (s:Student {id:$sid}), (t:Tag {id:$tid}) MERGE (s)-[:HAS_TAG]->(t)")
    ...
    private void runSafe(Runnable action) {
        try {
            ...
            log.warn("Neo4j 同步失败，已忽略，仍继续使用 MySQL 持久化", e);
        }
    }
}
